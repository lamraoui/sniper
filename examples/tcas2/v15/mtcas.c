...
bool Non_Crossing_Biased_Climb()
{
    ...
    {	
        result = Own_Above_Threat() && (Cur_Vertical_Sep >= MINSEP) && (Up_Separation >= ALIM()); printf("bug1\n");
    }
    return result;
}
...
bool Non_Crossing_Biased_Descend()
{
    ...
    if (upward_preferred)
    {
	result = Own_Below_Threat() && (Cur_Vertical_Sep >= MINSEP) && (Down_Separation >= ALIM()); printf("bug2\n");
    }
    else
    {
...
int alt_sep_test()
{
    ...
    enabled = High_Confidence && (Own_Tracked_Alt_Rate <= OLEV);  printf("bug3\n");
