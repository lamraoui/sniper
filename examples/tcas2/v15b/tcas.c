...
bool Non_Crossing_Biased_Climb()
{
    ...
    {	
	int MINSEP = 650; result = Own_Above_Threat() && (Cur_Vertical_Sep >= MINSEP) && (Up_Separation >= ALIM());
    }
    return result;
}
...
bool Non_Crossing_Biased_Descend()
{
    ...
    if (upward_preferred)
    {
	int MINSEP = 650; result = Own_Below_Threat() && (Cur_Vertical_Sep >= MINSEP) && (Down_Separation >= ALIM());
    }
    else
    {
