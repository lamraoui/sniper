...
bool Non_Crossing_Biased_Climb()
{
    ...
    if (upward_preferred)
    {
        ...
        if (Alt_Layer_Value==0) {
            r = Positive_RA_Alt_Thresh_0;
        }
        else if (Alt_Layer_Value==1) {
            r =  Positive_RA_Alt_Thresh_1;
        }
        else if (Alt_Layer_Value==2) {
            r = Positive_RA_Alt_Thresh_2;
        }
        else if (Alt_Layer_Value==3) {
            r =  Positive_RA_Alt_Thresh_3;
        }
	result = !((Own_Tracked_Alt < Other_Tracked_Alt)) || ( (Own_Tracked_Alt < Other_Tracked_Alt) && (!(Down_Separation >= r))); /* >= : opertor mutation */
    }
    else
    {	
        int r = 0;
	...
	result = (Other_Tracked_Alt < Own_Tracked_Alt) && (Cur_Vertical_Sep >= MINSEP) && (Up_Separation >= r);
    }
    return result;
}
...
bool Non_Crossing_Biased_Descend()
{
	...
	result = (Own_Tracked_Alt < Other_Tracked_Alt) && (Cur_Vertical_Sep >= MINSEP) && (Down_Separation >= r);
    }
    else
    {
        int r = 0;
	...
	result = !(Other_Tracked_Alt < Own_Tracked_Alt) || ((Other_Tracked_Alt < Own_Tracked_Alt) && (Up_Separation >= r));
    }
    return result;
}
