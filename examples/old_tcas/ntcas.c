...
char *argv[];
{
    ...
    if (enabled && ((tcas_equipped && intent_not_known) || !tcas_equipped))
    {
        ...
        bool result;
        {
            ...
            if (upward_preferred)
            {
                result = !((Own_Tracked_Alt < Other_Tracked_Alt) || ((Own_Tracked_Alt < Other_Tracked_Alt)) && (!(Down_Separation /* >= */ > (Positive_RA_Alt_Thresh[Alt_Layer_Value]))));
            }
            else
            {	
                result = (Other_Tracked_Alt < Own_Tracked_Alt) && (Cur_Vertical_Sep >= MINSEP) && (Up_Separation >= (Positive_RA_Alt_Thresh[Alt_Layer_Value]));
            }
        }
        ...
        bool result2;
        {
            int upward_preferred2;
            upward_preferred2 = (Climb_Inhibit ? Up_Separation + NOZCROSS : Up_Separation) > Down_Separation;
            if (upward_preferred2)
            {
                result2 = (Own_Tracked_Alt < Other_Tracked_Alt) && (Cur_Vertical_Sep >= MINSEP) && (Down_Separation >= (Positive_RA_Alt_Thresh[Alt_Layer_Value]));
            }
            else
            {
                result2 = !((Other_Tracked_Alt < Own_Tracked_Alt) || ((Other_Tracked_Alt < Own_Tracked_Alt)) && (Up_Separation >= (Positive_RA_Alt_Thresh[Alt_Layer_Value])));
            }
        }
        
        
        
        need_upward_RA = result && (Own_Tracked_Alt < Other_Tracked_Alt);
        need_downward_RA = result2 && (Other_Tracked_Alt < Own_Tracked_Alt);
