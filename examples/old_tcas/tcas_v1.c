...
bool Non_Crossing_Biased_Climb()
{
    ...
    if (upward_preferred)
    {
	result = !((Own_Tracked_Alt < Other_Tracked_Alt)) || (((Own_Tracked_Alt < Other_Tracked_Alt)) && (!(Down_Separation > ALIM()))); /* opertor mutation */
    }
    else
    {	
	result = (Other_Tracked_Alt < Own_Tracked_Alt) && (Cur_Vertical_Sep >= MINSEP) && (Up_Separation >= ALIM());
    }
    return result;
}
...
bool Non_Crossing_Biased_Descend()
{
    ...
    if (upward_preferred)
    {
	result = (Own_Tracked_Alt < Other_Tracked_Alt) && (Cur_Vertical_Sep >= MINSEP) && (Down_Separation >= ALIM());
    }
    else
    {
	result = !((Other_Tracked_Alt < Own_Tracked_Alt)) || (((Other_Tracked_Alt < Own_Tracked_Alt)) && (Up_Separation >= ALIM()));
    }
    return result;
}
